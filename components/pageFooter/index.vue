	...
	<view v-if="showTabBar" >
		<view class="fixed-lb w-full pb-safe z-999" :style="[bgColor]">
			<view class="page-footer-wrapper">
				...
				>
					<view :class="['foot-item', 'flex-1', 'flex-col', 'flex-center', 'h-96', 'relative', 'item-' + index]" v-for="(item, index) in newData.menuList" :key="index" @click="goRouter(item)">
						<template v-if="item.link.split('?')[0] == activeRouter">
							<image v-if="newData.navStyleConfig.tabVal != 1" :src="item.imgList[0]"></image>
							<view v-if="newData.navStyleConfig.tabVal != 2 && index !== 2" class="txt active" :style="[txtActiveColor]">{{ item.name }}</view>
						</template>
						<template v-else>
							<image v-if="newData.navStyleConfig.tabVal != 1" :src="item.imgList[1]"></image>
							<view v-if="newData.navStyleConfig.tabVal != 2 && index !== 2" class="txt" :style="[txtColor]">{{ item.name }}</view>
						</template>
